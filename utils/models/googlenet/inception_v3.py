...
def inception_v3(img_shape, num_classes=10, aux_logits=True, transform_input=True, pretrained=False):
    r"""Inception v3 model architecture from
    `"Rethinking the Inception Architecture for Computer Vision" <https://arxiv.org/abs/1512.00567>`_.
    """
    if pretrained:
        model = InceptionV3(img_shape, num_classes, aux_logits, True)
        load_state_dict(model, model_zoo.load_url(model_urls['inception_v3_google']))
    else:
        model = InceptionV3(img_shape, num_classes, aux_logits, transform_input)
    return model


class InceptionV3(nn.Module):
    def __init__(self, img_shape, num_classes=10, aux_logits=True, transform_input=False, resize=299):
        super().__init__()
        ...
        self.config = {'model': 'InceptionV3', 'img_shape': img_shape, 'num_classes': num_classes,
                       'aux_logits': aux_logits, 'transform_input': transform_input, 'resize': resize}

'''
    ...
    code:
        https://github.com/weiaicunzai/pytorch-cifar100/blob/master/models/nasnet.py
'''
...
def nasneta(img_shape, num_classes=1000):
    model = NasNetA(4, 2, 44, 44, img_shape, num_classes, )
    model.config.update({'model': 'NasNetA', })
    return model




class NasNetSepConv(nn.Module):
...
class NasNetSepBranch(nn.Module):
    def __init__(self, input_channels, output_channels, kernel_size, **kwargs):
        """Adds 2 blocks of [relu-separable conv-batchnorm]."""
        super().__init__()
        self.block1 = nn.Sequential(
            nn.ReLU(),
            NasNetSepConv(input_channels, output_channels, kernel_size, **kwargs),
            nn.BatchNorm2d(output_channels)
        )

        self.block2 = nn.Sequential(
            nn.ReLU(),
            NasNetSepConv(output_channels, output_channels, kernel_size, stride=1, padding=int(kernel_size / 2)),
            nn.BatchNorm2d(output_channels)
        )
...
class NasNetNormalCell(nn.Module):
    def __init__(self, x_in, prev_in, output_channels):
        ...
        self.block1_left = NasNetSepBranch(output_channels, output_channels, kernel_size=3, padding=1, bias=False)
        self.block1_right = nn.Sequential()

        self.block2_left = NasNetSepBranch(output_channels, output_channels, kernel_size=3, padding=1, bias=False)
        self.block2_right = NasNetSepBranch( output_channels, output_channels, kernel_size=5, padding=2, bias=False)
        ...
        self.block5_left = NasNetSepBranch( output_channels, output_channels, kernel_size=5, padding=2, bias=False)
        self.block5_right = NasNetSepBranch(output_channels, output_channels, kernel_size=3, padding=1, bias=False)

        self.fit = NasNetFit(prev_in, output_channels)
...
class NasNetReductionCell(nn.Module):
    def __init__(self, x_in, prev_in, output_channels):
        super().__init__()
        ...
        #block1
        self.layer1block1_left = NasNetSepBranch(output_channels, output_channels, 7, stride=2, padding=3)
        self.layer1block1_right = NasNetSepBranch(output_channels, output_channels, 5, stride=2, padding=2)

        #block2
        self.layer1block2_left = nn.MaxPool2d(3, stride=2, padding=1)
        self.layer1block2_right = NasNetSepBranch(output_channels, output_channels, 7, stride=2, padding=3)

        #block3
        self.layer1block3_left = nn.AvgPool2d(3, 2, 1)
        self.layer1block3_right = NasNetSepBranch(output_channels, output_channels, 5, stride=2, padding=2)

        #block5
        self.layer2block1_left = nn.MaxPool2d(3, 2, 1)
        self.layer2block1_right = NasNetSepBranch(output_channels, output_channels, 3, stride=1, padding=1)
...
class NasNetA(nn.Module):
    def __init__(self, repeat_cell_num, reduction_num, filters, stemfilter, img_shape=(3, 224, 224), num_classes=1000):
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(self.c, stemfilter, 3, padding=1, bias=False),
    ...
    def _make_layers(self, repeat_cell_num, reduction_num):
        ...
        for i in range(reduction_num):
            layers.extend(self._make_normal(NasNetNormalCell, repeat_cell_num, self.filters))
            self.filters *= 2
            layers.append(self._make_reduction(NasNetReductionCell, self.filters))

...
def xception(img_shape, num_classes=1000):
    model = Xception(MiddleFLowBlock, img_shape, num_classes)
    model.config.update({'model': 'Xception', })
    return model
...
class Xception(nn.Module):
    def __init__(self, block, img_shape=(3, 224, 224), num_classes=1000):
        super().__init__()
        self.c, self.h, self.w = img_shape
        self.img_shape = img_shape
        self.num_classes = num_classes
        self.config = {'model': 'Xception', 'img_shape': self.img_shape, 'num_classes': self.num_classes, }

        self.entry_flow = EntryFlow(self.c)

...
def shufflenetv2(img_shape, num_classes=1000, net_size=1):
    model = ShuffleNetV2(net_size, img_shape, num_classes, )
    model.config.update({'model': 'ShuffleNetV2', })
    return model
...
class ShuffleNetV2(nn.Module):
    def __init__(self, net_size=1, img_shape=(3, 224, 224), num_classes=1000):
        super().__init__()
        self.in_channels = 24
        ...
        self.config = {'model': 'ShuffleNetV2', 'img_shape': self.img_shape, 'num_classes': self.num_classes, 'net_size': net_size}

        self.conv1 = nn.Conv2d(self.c, 24, kernel_size=3, stride=1, padding=1, bias=False)

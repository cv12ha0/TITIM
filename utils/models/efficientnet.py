...
def efficientnetb0(img_shape, num_classes=1000):
    model = EfficientNetB0(img_shape, num_classes, )
    model.config.update({'model': 'EfficientNetB0', })
    return model
...
class EfficientNetBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, expand_ratio=1, se_ratio=0., drop_rate=0.):
        ...
        self.se = EfficientNetSE(channels, se_channels)
    ...
    def forward(self, x):
        ...
        if self.has_skip:
            # out = drop_connect(out, self.drop_rate) if (self.training and self.drop_rate > 0) else out
...
class EfficientNet(nn.Module):
    def __init__(self, cfg, img_shape=(3, 224, 224), num_classes=1000):
        ...
        self.config = {'model': 'EfficientNet', 'img_shape': self.img_shape, 'num_classes': self.num_classes, }
    ...
    def _make_layers(self, in_channels):
        ...
        for expansion, out_channels, num_blocks, kernel_size, stride in zip(*cfg):
            ...
            for stride in strides:
                ...
                layers.append(EfficientNetBlock(in_channels, out_channels, kernel_size, stride, expansion, se_ratio=0.25, drop_rate=drop_rate))
...
def EfficientNetB0(img_shape=(3, 224, 224), num_classes=1000):
    cfg = {
    ...
    return EfficientNet(cfg, img_shape, num_classes)

'''
    Resnet in Resnet: Generalizing Residual Architectures (https://arxiv.org/abs/1603.08029v1)

    code:
        https://github.com/weiaicunzai/pytorch-cifar100/blob/master/models/rir.py
'''
import torch
...
def rir(img_shape, num_classes=1000):
    model = ResnetInResneet(img_shape, num_classes)
    model.config.update({'model': 'RIR', })  # ResnetInResneet
    return model
...
class ResnetInResneet(nn.Module):
    def __init__(self, img_shape=(3, 224, 224), num_classes=1000):
        ...
        self.config = {'model': 'RIR', 'img_shape': self.img_shape, 'num_classes': self.num_classes, }

        self.residual_pre_conv = nn.Sequential(
            nn.Conv2d(self.c, base, 3, padding=1),
            nn.BatchNorm2d(base),
            nn.ReLU(inplace=True)
        )
        ...
        self.fc = nn.Sequential(
            nn.Linear(9*num_classes, 9*num_classes // 2),  # nn.Linear(900, 450)
            nn.ReLU(),
            nn.Dropout(),
            nn.Linear(9*num_classes // 2, num_classes),  # nn.Linear(450, 100)
        )

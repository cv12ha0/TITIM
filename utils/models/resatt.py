'''
    Residual Attention Network for Image Classification (https://openaccess.thecvf.com/content_cvpr_2017/html/Wang_Residual_Attention_Network_CVPR_2017_paper.html)

    code:
        https://github.com/weiaicunzai/pytorch-cifar100/blob/master/models/attention.py
'''
import torch
from torch import nn
import torch.nn.functional as F
__all__ = ["ResAttention", "resatt56", "resatt92"]


def resatt56(img_shape, num_classes=1000):
    model = ResAttention([1, 1, 1], img_shape, num_classes)
    model.config.update({'model': 'ResAtt56', })
    return model


def resatt92(img_shape, num_classes=1000):
    model = ResAttention([1, 2, 3], img_shape, num_classes)
    model.config.update({'model': 'ResAtt92', })
    return model
...
class ResAttention(nn.Module):
    ...
    """
    def __init__(self, block_num, img_shape=(3, 224, 224), num_classes=1000):
        super().__init__()
        self.c, self.h, self.w = img_shape
        self.img_shape = img_shape
        self.num_classes = num_classes
        self.config = {'model': 'ResAttention', 'img_shape': self.img_shape, 'num_classes': self.num_classes, }
